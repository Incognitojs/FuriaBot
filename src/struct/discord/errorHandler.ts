...
export default class ErrorHandler {
    ...
    roleExists = (interaction: Iinteraction, role: string) =>
        interaction.reply({
            content: `> ${this.client.Iemojis.error} The role **${role}** already exists within this guild.`,
            ephemeral: true
        })

    roleCreate = (interaction: Iinteraction, role: string) =>
        interaction.reply({
    ...
    alreadyMuted = (interaction: Iinteraction) =>
        interaction.reply({
            content: `> ${this.client.Iemojis.error} This user is already **muted**`,
            ephemeral: true
        })

    mute = (interaction: Iinteraction) =>
        interaction.reply({
